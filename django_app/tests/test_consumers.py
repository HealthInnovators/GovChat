...
from GovChat.graph.root import FINAL_RESPONSE_TAG, ROUTE_NAME_TAG, SOURCE_DOCUMENTS_TAG
from GovChat.models.chat import MetadataDetail
from GovChat_app.GovChat_core import error_messages
from GovChat_app.GovChat_core.consumers import ChatConsumer
from GovChat_app.GovChat_core.models import Chat, ChatMessage, ChatMessageTokenUse, ChatRoleEnum, File
from GovChat_app.GovChat_core.prompts import CHAT_MAP_QUESTION_PROMPT
...
async def test_chat_consumer_with_new_session(alice: User, uploaded_file: File, mocked_connect: Connect):
    ...
    # When
    with patch("GovChat_app.GovChat_core.consumers.ChatConsumer.GovChat.graph", new=mocked_connect):
...
async def test_chat_consumer_with_naughty_citation(
    ...
    # When
    with patch("GovChat_app.GovChat_core.consumers.ChatConsumer.GovChat.graph", new=mocked_connect_with_naughty_citation):
        communicator = WebsocketCommunicator(ChatConsumer.as_asgi(), "/ws/chat/")
        communicator.scope["user"] = alice
        connected, _ = await communicator.connect()
        assert connected
...
async def test_chat_consumer_with_selected_files(
    ...
    # When
    with patch("GovChat_app.GovChat_core.consumers.ChatConsumer.GovChat.graph", new=mocked_connect_with_several_files):
        communicator = WebsocketCommunicator(ChatConsumer.as_asgi(), "/ws/chat/")
        communicator.scope["user"] = alice
        connected, _ = await communicator.connect()
        assert connected
...
async def test_chat_consumer_with_connection_error(alice: User, mocked_breaking_connect: Connect):
    ...
    # When
    with patch("GovChat_app.GovChat_core.consumers.ChatConsumer.GovChat.graph", new=mocked_breaking_connect):
        communicator = WebsocketCommunicator(ChatConsumer.as_asgi(), "/ws/chat/")
        communicator.scope["user"] = alice
        connected, _ = await communicator.connect()
        assert connected
...
async def test_chat_consumer_with_explicit_unhandled_error(
    ...
    # When
    with patch(
        "GovChat_app.GovChat_core.consumers.ChatConsumer.GovChat.graph", new=mocked_connect_with_explicit_unhandled_error
    ):
        communicator = WebsocketCommunicator(ChatConsumer.as_asgi(), "/ws/chat/")
        communicator.scope["user"] = alice
        connected, _ = await communicator.connect()
...
async def test_chat_consumer_with_rate_limited_error(alice: User, mocked_connect_with_rate_limited_error: Connect):
    ...
    # When
    with patch(
        "GovChat_app.GovChat_core.consumers.ChatConsumer.GovChat.graph", new=mocked_connect_with_rate_limited_error
    ):
        communicator = WebsocketCommunicator(ChatConsumer.as_asgi(), "/ws/chat/")
        communicator.scope["user"] = alice
        connected, _ = await communicator.connect()
...
async def test_chat_consumer_with_explicit_no_document_selected_error(
    ...
    # When
    with patch(
        "GovChat_app.GovChat_core.consumers.ChatConsumer.GovChat.graph",
