...
def test_0012_alter_file_status(migrator):
    old_state = migrator.apply_initial_migration(("GovChat_core", "0012_alter_file_status"))

    original_file = SimpleUploadedFile("original_file.txt", b"Lorem Ipsum.")

    User = old_state.apps.get_model("GovChat_core", "User")
    ChatHistory = old_state.apps.get_model("GovChat_core", "ChatHistory")
    ChatMessage = old_state.apps.get_model("GovChat_core", "ChatMessage")
    File = old_state.apps.get_model("GovChat_core", "File")
    ...
    new_state = migrator.apply_tested_migration(
        ("GovChat_core", "0013_chatmessage_selected_files_and_more"),
    )
    NewChatMessage = new_state.apps.get_model("GovChat_core", "ChatMessage")  # noqa: N806
...
def test_0020_remove_chatmessage_source_files_textchunk_and_more(migrator):
    old_state = migrator.apply_initial_migration(("GovChat_core", "0019_alter_chatmessage_route"))
    ...
    assert chat_message.source_files.first().pk == file.pk
    new_state = migrator.apply_tested_migration(
        ("GovChat_core", "0020_remove_chatmessage_source_files_textchunk_and_more"),
    )
    NewChatMessage = new_state.apps.get_model("GovChat_core", "ChatMessage")  # noqa: N806
    Citation = new_state.apps.get_model("GovChat_core", "Citation")
...
def test_0027_alter_file_status(migrator):
    ...
    old_state = migrator.apply_initial_migration(("GovChat_core", "0026_alter_file_status"))
    ...
    new_state = migrator.apply_tested_migration(
        ("GovChat_core", "0027_alter_file_status"),
    )
    NewFile = new_state.apps.get_model("GovChat_core", "File")  # noqa: N806
...
def test_0028_aisettings(migrator):
    old_state = migrator.apply_initial_migration(("GovChat_core", "0027_alter_file_status"))

    User = old_state.apps.get_model("GovChat_core", "User")
    User.objects.create(email="someone@example.com")

    new_state = migrator.apply_tested_migration(
        ("GovChat_core", "0028_aisettings"),
    )
    NewUser = new_state.apps.get_model("GovChat_core", "User")  # noqa: N806
...
def test_0029_rename_chathistory_chat_alter_chat_options(migrator):
    old_state = migrator.apply_initial_migration(("GovChat_core", "0028_aisettings"))
    ...
    new_state = migrator.apply_tested_migration(
        ("GovChat_core", "0029_rename_chathistory_chat_alter_chat_options"),
    )
    Chat = new_state.apps.get_model("GovChat_core", "Chat")
...
def test_0030_chatmessagerating_chips(migrator):
    old_state = migrator.apply_initial_migration(("GovChat_core", "0029_rename_chathistory_chat_alter_chat_options"))

    User = old_state.apps.get_model("GovChat_core", "User")
    user = User.objects.create(email="someone@example.com")

    Chat = old_state.apps.get_model("GovChat_core", "Chat")
    chat = Chat.objects.create(users=user, name="my-chat")

    ChatMessage = old_state.apps.get_model("GovChat_core", "ChatMessage")
    chat_message = ChatMessage.objects.create(chat=chat)

    ChatMessageRating = old_state.apps.get_model("GovChat_core", "ChatMessageRating")
    chat_message_rating = ChatMessageRating.objects.create(chat_message=chat_message, rating=3, text="very average")

    ChatMessageRatingChip = old_state.apps.get_model("GovChat_core", "ChatMessageRatingChip")
    ...
    new_state = migrator.apply_tested_migration(
        ("GovChat_core", "0030_chatmessagerating_chips"),
    )
    NewChatMessageRating = new_state.apps.get_model("GovChat_core", "ChatMessageRating")  # noqa: N806
...
def test_0031_chatmessage_rating_chatmessage_rating_chips_and_more(migrator):
    old_state = migrator.apply_initial_migration(("GovChat_core", "0030_chatmessagerating_chips"))
    ...
    new_state = migrator.apply_tested_migration(
        ("GovChat_core", "0031_chatmessage_rating_chatmessage_rating_chips_and_more"),
    )
    NewChatMessage = new_state.apps.get_model("GovChat_core", "ChatMessage")  # noqa: N806
    new_chat_message = NewChatMessage.objects.get(pk=chat_message.pk)
...
def test_0032_user_new_business_unit(migrator):
    ...
    BusinessUnit = old_state.apps.get_model("GovChat_core", "BusinessUnit")
    ...
    new_state = migrator.apply_tested_migration(
        ("GovChat_core", "0032_user_new_business_unit"),
    )
    NewUser = new_state.apps.get_model("GovChat_core", "User")  # noqa: N806
...
def test_0042_chat_chat_backend_chat_chat_map_question_prompt_and_more(migrator):
    old_state = migrator.apply_initial_migration(("GovChat_core", "0041_alter_aisettings_chat_backend"))
    ...
    new_state = migrator.apply_tested_migration(
        ("GovChat_core", "0042_chat_chat_backend_chat_chat_map_question_prompt_and_more"),
    )

    new_chat_model = new_state.apps.get_model("GovChat_core", "Chat")
