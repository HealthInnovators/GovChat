...
from GovChat.models.file import ChunkMetadata, ChunkResolution
...
class GovChatChatTestCase:
    def __init__(
        self,
        test_id: str,
        query: GovChatQuery,
        test_data: GovChatTestData,
...
def generate_test_cases(query: GovChatQuery, test_data: list[GovChatTestData], test_id: str) -> list[GovChatChatTestCase]:
    return [
        GovChatChatTestCase(test_id=f"{test_id}-{i}", query=query, test_data=data) for i, data in enumerate(test_data)
    ]
