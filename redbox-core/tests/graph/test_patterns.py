...
from GovChat.graph.nodes.processes import (
    build_chat_pattern,
    build_merge_pattern,
...
from GovChat.chains.runnables import build_chat_prompt_from_messages_runnable, build_llm_chain, CannedChatLLM
from GovChat.test.data import (
    GovChatTestData,
    GovChatChatTestCase,
    generate_test_cases,
    generate_docs,
...
from GovChat.models.chain import PromptSet, GovChatQuery, GovChatState
from GovChat.transform import flatten_document_state, structure_documents


LANGGRAPH_DEBUG = True

CHAT_PROMPT_TEST_CASES = generate_test_cases(
    query=GovChatQuery(question="What is AI?", s3_keys=[], user_uuid=uuid4(), chat_history=[]),
    test_data=[
        GovChatTestData(0, 0, expected_llm_response=["Testing Response 1"], expected_route=ChatRoute.chat),
        GovChatTestData(
...
def test_build_chat_prompt_from_messages_runnable(test_case: GovChatChatTestCase, tokeniser: Encoding):
    ...
    state = GovChatState(request=test_case.query, documents=test_case.docs)
...
BUILD_LLM_TEST_CASES = generate_test_cases(
    query=GovChatQuery(question="What is AI?", file_uuids=[], user_uuid=uuid4(), chat_history=[]),
    test_data=[
        GovChatTestData(
            2, 40_000, expected_llm_response=["Testing Response 1"], expected_route=ChatRoute.chat_with_docs
        ),
...
def test_build_llm_chain(test_case: GovChatChatTestCase):
...
CHAT_TEST_CASES = generate_test_cases(
    query=GovChatQuery(question="What is AI?", s3_keys=[], user_uuid=uuid4(), chat_history=[]),
    test_data=[GovChatTestData(0, 0, expected_llm_response=["Testing Response 1"], expected_route=ChatRoute.chat)],
    test_id="Chat pattern",
)


@pytest.mark.parametrize(("test_case"), CHAT_TEST_CASES, ids=[t.test_id for t in CHAT_TEST_CASES])
def test_build_chat_pattern(test_case: GovChatChatTestCase, mocker: MockerFixture):
    """Tests a given state["request"] correctly changes state["text"]."""
    llm = GenericFakeChatModel(messages=iter(test_case.test_data.expected_llm_response))
    state = GovChatState(request=test_case.query, documents=[])

    chat = build_chat_pattern(prompt_set=PromptSet.Chat, final_response_chain=True)

    with (
        mocker.patch("GovChat.graph.nodes.processes.get_chat_llm", return_value=llm),
    ):
        response = chat(state)
    final_state = GovChatState(response)
...
def test_build_set_route_pattern(test_case: GovChatChatTestCase):
...
RETRIEVER_TEST_CASES = generate_test_cases(
    query=GovChatQuery(question="What is AI?", s3_keys=["s3_key_1", "s3_key_2"], user_uuid=uuid4(), chat_history=[]),
    test_data=[
        GovChatTestData(
            2, 40_000, expected_llm_response=["Testing Response 1"], expected_route=ChatRoute.chat_with_docs
        ),
...
def test_build_retrieve_pattern(test_case: GovChatChatTestCase, mock_retriever: BaseRetriever):
...
def test_build_merge_pattern(test_case: GovChatChatTestCase, mocker: MockerFixture):
    """Tests a given state["request"] and state["documents"] correctly changes state["documents"]."""
    llm = GenericFakeChatModel(messages=iter(test_case.test_data.expected_llm_response))
    state = GovChatState(request=test_case.query, documents=structure_documents(test_case.docs))
...
def test_build_stuff_pattern(test_case: GovChatChatTestCase, mocker: MockerFixture):
...
def test_build_passthrough_pattern():
    ...
    state = GovChatState(
        request=GovChatQuery(question="What is AI?", s3_keys=[], user_uuid=uuid4(), chat_history=[]),
    )
...
def test_empty_process():
    ...
    builder = StateGraph(GovChatState)
...
CLEAR_DOC_TEST_CASES = [
    GovChatState(
        request=GovChatQuery(question="What is AI?", file_uuids=[], user_uuid=uuid4(), chat_history=[]),
        documents=structure_documents([doc for doc in generate_docs(s3_key="s3_key")]),
        text="Foo",
        route_name=ChatRoute.chat_with_docs_map_reduce,
    ),
...
def test_clear_documents(test_case: list[GovChatState]):
