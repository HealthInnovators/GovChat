...
resource "aws_iam_role_policy_attachment" "GovChat_role_policy" {
  ...
  role       = each.value
  policy_arn = aws_iam_policy.GovChat_policy.arn
}
...
data "aws_iam_policy_document" "github-runner" {
  ### Allow access to GovChat specific s3 buckets
  statement {
    effect  = "Allow"
    actions = ["s3:*"]
    resources = [
