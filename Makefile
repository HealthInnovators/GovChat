...
test-ai: ## Test code with live LLM
	cd GovChat-core && poetry install --with dev && poetry run python -m pytest -m "ai" --cov=GovChat -v --cov-report=term-missing --cov-fail-under=80

.PHONY: test-GovChat
test-GovChat: ## Test GovChat
	cd GovChat-core && poetry install && poetry run pytest -m "not ai" --cov=GovChat -v --cov-report=term-missing --cov-fail-under=60

.PHONY: test-django
test-django: ## Test django-app
	cd django_app && poetry install && poetry run pytest --cov=GovChat_app -v --cov-report=term-missing --cov-fail-under=60 --ds GovChat_app.settings
...
.PHONY: safe
safe:  ##
	poetry run bandit -ll -r ./GovChat
	poetry run bandit -ll -r ./django_app

.PHONY: check-migrations
check-migrations: stop  ## Check types in GovChat and worker
...
ifeq ($(instance),postgres)
 CONFIG_DIR=../../../../GovChat-copilot-infra-config
 tf_build_args=
