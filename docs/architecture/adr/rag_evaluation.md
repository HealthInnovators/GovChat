...
- \[ADR in review]\(https://github.com/i-dot-ai/GovChat/pull/330) 2024-05-13
...
- We need robust LLM evaluation in place, to give ourselves and users confidence that GovChat is working as it should and to avoid undesirable side effects of using LLMs, such as hallucinations that would reduce trust in the tools we are building.
...
- Bridges unit testing and evaluation for GovChat
...
- Using `DeepEval` hosted platform on Confident AI is not a dealbreaker at the moment, so we can continue with using `DeepEval` for evaluating GovChat chat
...
- \[Title\]\(URL\) --> 
[Link to GovChat RAG evaluation notebook for this spike](../../../notebooks/evaluation/GovChat_rag_evaluation.ipynb)