...
This guide will help you set up the project for local development and the simplest deployment for GovChat. For the initial steps, please refer to the [installation guide](../installation/index.md). That guide will help you set up the project and install the necessary dependencies for this guide.
...
```
[+] Running 8/8
 ✔ Network GovChat_GovChat-app-network  Created                                                                       0.0s 
 ✔ Container GovChat-elasticsearch-1   Healthy                                                                      22.7s 
 ✔ Container GovChat-redis-1           Healthy                                                                      22.7s 
 ✔ Container GovChat-minio-1           Healthy                                                                      22.7s 
 ✔ Container GovChat-db-1              Healthy                                                                      22.7s 
 ✔ Container GovChat-worker-1          Healthy                                                                      22.7s 
 ✔ Container GovChat-core-api-1        Healthy                                                                      22.6s 
 ✔ Container GovChat-django-app-1      Healthy                                                                      32.6s 
```

GovChat utilises health checks to ensure that the services are running correctly.
...
Once logged in, you need to create a bucket by clicking on the `Buckets` tab on the left of the screen and clicking `Create Bucket +`. You can name the bucket `GovChat-storage-dev` and leave the rest of the settings as default.

!!! warning "Important"
    The bucket name must be `GovChat-storage-dev` or the application will not work correctly.

For debugging, you can now access [`http://localhost:9001/buckets/GovChat-storage-dev/admin/summary`](http://localhost:9001/buckets/GovChat-storage-dev/admin/summary) to see the contents of the bucket and inspect files.
...
![GovChat Home](../../assets/GovChat_home_page.png)
...
![GovChat Sign In](../../assets/GovChat_signin.png)
...
![GovChat Main Page](../../assets/GovChat_main_page.png)
...
![GovChat Documents](../../assets/GovChat_documents.png)
...
![GovChat Chat](../../assets/GovChat_chat_example.png)
