...
- [GovChat-core](https://github.com/i-dot-ai/GovChat/blob/main/GovChat-core/pyproject.toml) - core AI package
- [django-app](https://github.com/i-dot-ai/GovChat/blob/main/django_app/pyproject.toml) - django webserver and background worker
- [root](https://github.com/i-dot-ai/GovChat/blob/main/pyproject.toml) - Integration tests, QA, and docs


## VSCode
To make use of the VSCode setup open the workspace file .vscode/GovChat.code-workspace. This will open the relevant services as roots in a single workspace. The recommended way to use this is:
* Create a venv in each of the main service directories (core-api, GovChat-core, worker) this should be in a directory called _venv_
...
### Backend Profiles
GovChat can use different backends for chat and embeddings, which are used is controlled by env vars. The defaults are currently to use Azure for both chat and embeddings but OpenAI can be used (and pointed to an OpenAI compliant local service).
...
To view all the build commands, check the `Makefile` that can be found [here](https://github.com/i-dot-ai/GovChat/blob/main/Makefile).
...
For any other commands available, check the `Makefile` [here](https://github.com/i-dot-ai/GovChat/blob/main/Makefile).
...
The default indicex we want is `GovChat-data-chunk`

Dump these to [data/elastic-dumps/](../data/elastic-dumps/) for saving or sharing.

```console
elasticdump \
  --input=http://localhost:9200/GovChat-data-chunk \
  --output=./data/elastic-dumps/GovChat-data-chunk.json \
  --type=data
```

### Loading data to Elasticsearch

If you've been provided with a dump from the vector store, add it to [data/elastic-dumps/](../data/elastic-dumps/). The below assumes the existance of `GovChat-data-chunk.json` in that directory.
...
```console
elasticdump \
  --input=./data/elastic-dumps/GovChat-data-chunk.json \
  --output=http://localhost:9200/GovChat-data-chunk \
  --type=data
```
