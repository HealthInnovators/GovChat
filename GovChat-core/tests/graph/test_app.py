...
from GovChat.models.chain import GovChatQuery, GovChatState
from GovChat import GovChat
...
from GovChat.models.chain import metadata_reducer
    ...
    for generated_cases in [
        generate_test_cases(
            ...
            test_data=[
                ...
                GovChatTestData(1, 100, expected_llm_response=["Testing Response 1"], expected_route=ChatRoute.chat),
                GovChatTestData(10, 1200, expected_llm_response=["Testing Response 1"], expected_route=ChatRoute.chat),
            ],
            test_id="Basic Chat",
        ),
        generate_test_cases(
            query=GovChatQuery(question="What is AI?", s3_keys=["s3_key"], user_uuid=uuid4(), chat_history=[]),
            test_data=[
                GovChatTestData(
        ...
        generate_test_cases(
            query=GovChatQuery(question="@search What is AI?", s3_keys=["s3_key"], user_uuid=uuid4(), chat_history=[]),
            test_data=[
                GovChatTestData(
                    1,
                    10000,
        ...
        generate_test_cases(
            query=GovChatQuery(question="@nosuchkeyword What is AI?", s3_keys=[], user_uuid=uuid4(), chat_history=[]),
            test_data=[
                GovChatTestData(
                    10,
                    1000,
...
async def test_chat(test: GovChatChatTestCase, env: Settings, mocker: MockerFixture):
    ...
    with (
        ...
        response = await app.run(input=GovChatState(request=test_case.query))
...
@pytest.mark.asyncio
@pytest.mark.parametrize(("test"), TEST_CASES, ids=[t.test_id for t in TEST_CASES])
async def test_streaming(test: GovChatChatTestCase, env: Settings, mocker: MockerFixture):
    # Current setup modifies test data as it's not a fixture. This is a hack
    test_case = copy.deepcopy(test)
    ...
    with (
        ...
        response = await app.run(
            input=GovChatState(request=test_case.query),
