...
from GovChat.graph.root import get_root_graph
...
from GovChat.chains.components import get_all_chunks_retriever, get_parameterised_retriever
from GovChat.graph.root import (
    ROUTABLE_KEYWORDS,
    ROUTE_NAME_TAG,
    FINAL_RESPONSE_TAG,
    ...
    async def run(
        ...
        async for event in self.graph.astream_events(input, version="v2"):
            ...
            if kind == "on_chat_model_stream" and FINAL_RESPONSE_TAG in tags:
                ...
                final_state = GovChatState(**event["data"]["output"])
        return final_state
