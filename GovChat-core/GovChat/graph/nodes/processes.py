...
from GovChat.chains.components import get_tokeniser, get_chat_llm
from GovChat.chains.runnables import build_llm_chain, CannedChatLLM
from GovChat.models import ChatRoute, Settings
from GovChat.models.chain import GovChatState
from GovChat.transform import combine_documents, structure_documents
from GovChat.models.chain import PromptSet
from GovChat.transform import flatten_document_state
...
def build_retrieve_pattern(
...
) -> Callable[[GovChatState], dict[str, Any]]:
    ...
    def _retrieve(state: GovChatState) -> dict[str, Any]:
...
def build_chat_pattern(
    ...
    def _chat(state: GovChatState) -> dict[str, Any]:
...
def build_merge_pattern(
    ...
    def _merge(state: GovChatState) -> dict[str, Any]:
        llm = get_chat_llm(Settings(), state["request"].ai_settings)
...
def build_stuff_pattern(
    ...
    def _stuff(state: GovChatState) -> dict[str, Any]:
        llm = get_chat_llm(Settings(), state["request"].ai_settings)
...
def build_set_route_pattern(route: ChatRoute) -> Callable[[GovChatState], dict[str, Any]]:
    """Returns a function that sets state["route_name"]."""

    def _set_route(state: GovChatState) -> dict[str, Any]:
...
def build_passthrough_pattern() -> Callable[[GovChatState], dict[str, Any]]:
    """Returns a function that uses state["request"] to set state["text"]."""

    def _passthrough(state: GovChatState) -> dict[str, Any]:
        return {
...
def build_set_text_pattern(text: str, final_response_chain: bool = False):
    ...
    def _set_text(state: GovChatState) -> dict[str, Any]:
...
def clear_documents_process(state: GovChatState) -> dict[str, Any]:
    if documents := state.get("documents"):
        return {"documents": {group_id: None for group_id in documents}}


def empty_process(state: GovChatState) -> None:
    return None
