...
from GovChat.models.chain import get_prompts
from GovChat.chains.components import get_tokeniser
from GovChat.graph.nodes.processes import PromptSet
from GovChat.models import ChatRoute
from GovChat.models.chain import GovChatState
from GovChat.transform import get_document_token_count

log = logging.getLogger()


def calculate_token_budget(state: GovChatState, system_prompt: str, question_prompt: str) -> int:
...
def build_documents_bigger_than_context_conditional(prompt_set: PromptSet) -> Runnable:
    ...
    def _documents_bigger_than_context_conditional(state: GovChatState) -> bool:
        system_prompt, question_prompt = get_prompts(state, prompt_set)
...
def documents_bigger_than_n_conditional(state: GovChatState) -> bool:
...
def build_keyword_detection_conditional(*allowed_routes: ChatRoute) -> Runnable:
    ...
    def _keyword_detection_conditional(state: GovChatState) -> ChatRoute | str:
...
def documents_selected_conditional(state: GovChatState) -> bool:
    return len(state["request"].s3_keys) > 0


def multiple_docs_in_group_conditional(state: GovChatState) -> bool:
